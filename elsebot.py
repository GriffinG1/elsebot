...
if not isfile("database/config.json"):
    with open("database/config.json", "w") as f:
        dump({'prefix': [".", "sudo ","hey taiga "], 'token': ''}, f,
...
async def on_ready():
    ...
    for guild in bot.guilds:
        ...
        # Moderation Roles
        bot.owner_role = get(guild.roles, name="owners")
        bot.admin_role = get(guild.roles, name="admins")
        bot.mod_role = get(guild.roles, name="moderators")
        ...
        bot.botdev_channel = get(guild.channels, name="botdev-place")
